...
const ReservationsEnhanced = () => {
  ...
  const loadReservations = useCallback(async () => {
    try {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Could not load reservations",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  // Dialog management
  const openDialog = (dialogName: keyof typeof dialogStates, reservation?: Reservation) => {
  ...
  const isUserJoined = (reservation: Reservation): boolean => {
    return !!reservation.lineup?.some(player => player.userId === currentUserId);
  };
  ...
  const isFull = (reservation: Reservation): boolean => {
    return (reservation.lineup?.length || 0) >= (reservation.maxPlayers + 2);
  };
  ...
  const handleLeaveGame = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Failed to leave the game",
        variant: "destructive",
      });
    }
  };
  ...
  const handleCompleteGame = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Failed to complete the game",
        variant: "destructive",
      });
    }
  };
  ...
  const handleSuspendPlayer = async (playerId: string, suspensionDays: number, reason: string) => {
    try {
  ...
  const filteredReservations = currentDate 
    ? reservations.filter(res => res.date === format(currentDate, 'yyyy-MM-dd'))
    : reservations.filter(res => new Date(res.date) >= today);

  const upcomingReservations = filteredReservations.filter(res => res.status === 'upcoming');
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:sticky lg:top-20 h-fit">
          <EnhancedDatePicker
            ...
            hasReservations={(date) => reservations.some(res => res.date === format(date, 'yyyy-MM-dd'))}
          />
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div>
            <div className="flex items-center mb-4">
              ...
              <h2 className="text-xl font-semibold text-teal-600">Upcoming Games</h2>
            </div>
              ...
              <div className="grid gap-4">
                {upcomingReservations.map((reservation) => (
                  <ReservationCardEnhanced
                    ...
                    onJoinWaitlist={() => openDialog('joinWaitlist', reservation)}
                    onLeaveWaitlist={() => openDialog('leaveWaitlist', reservation)}
                    onViewDetails={() => openDialog('gameDetails', reservation)}
                    onDeleteReservation={() => openDialog('deleteReservation', reservation)}
                    onCompleteGame={() => openDialog('completeGame', reservation)}
          ...
          {userRole === 'admin' && (
            <div>
              <div className="flex items-center mb-4">
                ...
                <h2 className="text-xl font-semibold text-blue-600">Completed Games</h2>
              </div>
      ...
      {selectedReservation && (
        <>
          ...
          <GameDetailsDialog
            ...
            currentUserId={currentUserId || ""}
            actualMaxPlayers={selectedReservation.maxPlayers + 2}
